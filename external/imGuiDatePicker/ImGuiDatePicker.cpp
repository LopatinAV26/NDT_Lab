...
namespace ImGui
{
    ...
    // Родительный падеж - для строки вида "2 августа 2026" (используется только в TimePointToLongString)
    static const std::vector<std::string> MONTHS_GENITIVE =
    {
        "января",
        "февраля",
        "марта",
    ...
    inline static std::string TimePointToLongString(const tm& timePoint) noexcept
    {
        ...
        std::string month = MONTHS_GENITIVE[GET_MONTH_UNSCALED(timePoint)];
    ...
    inline static tm Today() noexcept
    {
        ...
        tm res{};
#if defined(_WIN32)
        localtime_s(&res, &currentTime); // dest, src
#else
        localtime_r(&currentTime, &res); // src, dest
#endif
    ...
    bool DatePickerEx(const std::string& label, tm& v, ImFont* altFont, bool clampToBorder, float itemSpacing)
    {
        ...
        if (BeginCombo(std::string("##" + myLabel).c_str(), TimePointToLongString(v).c_str()))
        {
            ...
            SetItemTooltip("Установить текущую дату");
